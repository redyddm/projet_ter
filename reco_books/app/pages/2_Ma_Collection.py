...
if not st.session_state.get("logged_in", False):
    ...
    if st.button("Se connecter", key="login"):
        if user_index_input in ratings["user_index"].values:
            st.session_state["logged_in"] = True
            st.session_state["user_index"] = user_index_input
            st.session_state["user_id"] = ratings.loc[ratings["user_index"] == user_index_input, "user_id"].iloc[0]
        else:
...
if not st.session_state.get("logged_in", False):
    st.warning("👉 Connectez-vous pour voir votre bibliothèque.")
else:
