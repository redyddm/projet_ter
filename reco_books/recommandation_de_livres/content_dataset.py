...
def main(
    ...
    output_path_pkl: Path = PROCESSED_DATA_DIR / "content_dataset.pkl",
):
    logger.info("Loading raw datasets...")
    books = load_content.load_books(input_path)
    authors = load_content.load_authors(authors_path)
    categories = load_content.load_categories(categories_path)

    logger.info("Building content-based dataset...")
    books_df = build_dataset_content.build_content_dataset(books, authors, categories)
