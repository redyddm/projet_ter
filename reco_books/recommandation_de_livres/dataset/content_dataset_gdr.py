...
from recommandation_de_livres.build_dataset import build_content_dataset_gdr
...
@app.command()
def main(
    input_path: Path = RAW_DATA_DIR / DIR / "books.csv",
    ...
    books = load_data.load_csv(input_path)
    authors = load_data.load_csv(authors_path)
    categories = load_data.load_csv(categories_path)

    logger.info("Building content-based dataset...")
    books_df = build_content_dataset_gdr.build_content_dataset(books, authors, categories)
